...
function updateAdapterSummary(adapters) {
    const normalized = normalizeAdapters(adapters);
    const summaryEl = document.getElementById('consultant-selector-summary');
    const badgeEl = document.getElementById('adapter-count-badge');
    if (summaryEl) {
        const prefix = normalized.length === 1 ? 'Активен' : 'Активны';
        summaryEl.textContent = `${prefix}: ${normalized.map((key) => ADAPTER_INFO[key].label).join(', ')}`;
    }
    if (badgeEl) {
        badgeEl.textContent = `${normalized.length}/${Object.keys(ADAPTER_INFO).length}`;
    }
}
function setSelectedAdapters(adapters, forceHeaderUpdate = false) {
    ...
    document.querySelectorAll('.consultant-btn').forEach((btn) => {
        const isActive = selectedAdapters.includes(btn.dataset.adapter);
...
function addMessage(text, role, scroll = true) {
    ...
    if (role === 'assistant') {
        messageDiv.innerHTML = typeof marked !== 'undefined' ? marked.parse(text) : escapeHtml(text).replace(/\n/g, '<br>');
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.consultant-btn').forEach((btn) => {
        btn.addEventListener('click', () => toggleAdapter(btn.dataset.adapter));
    });
